...
begin;
  set local role anon;
  select iniciar_ronda(null);                 -- esperado: permission denied
rollback;

begin;
  set local role anon;
  select count(*) from ronda_ejecuciones;     -- esperado: permission denied
rollback;
...
begin;
  set local role anon;
  select count(*) from ronda_ejecucion_puntos; -- esperado: permission denied
rollback;
