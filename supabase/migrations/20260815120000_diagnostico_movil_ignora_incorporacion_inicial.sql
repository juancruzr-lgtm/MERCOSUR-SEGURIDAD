-- ============================================================================
-- CORRECTIVO · La incorporación inicial no es una mudanza
-- ============================================================================
--
-- QUÉ ESTABA MAL
--
-- 20260815110000 recorta la ventana del diagnóstico al `vigente_desde` de la
-- ubicación vigente cuando el objetivo es móvil. La idea es correcta: no hay
-- que mezclar fichajes de antes y después de una mudanza.
--
-- Pero la primera vigencia de todos los objetivos existentes es la
-- `incorporacion_inicial` que creó la migración de vigencias, con fecha del día
-- en que se aplicó. Eso NO es una mudanza: es la fecha en que empezamos a
-- registrar el historial. El objetivo puede llevar meses donde está.
--
-- Consecuencia: al marcar un objetivo como móvil, el diagnóstico recortaba la
-- ventana al día de la migración, encontraba cero fichajes y respondía
-- "sin datos · últimos 1 días". Todos los móviles quedaban ciegos hasta juntar
-- evidencia nueva.
--
-- QUÉ CAMBIA
--
-- El recorte se aplica sólo cuando la vigencia describe un cambio real de
-- ubicación:
--
--   manual · diagnostico_gps · alta   → sí recorta. Hubo una mudanza o el
--                                       objetivo nació ahí.
--   incorporacion_inicial             → NO recorta. No sabemos desde cuándo
--                                       rige, así que se usa la ventana normal,
--                                       igual que en un objetivo fijo.
--
-- En cuanto un móvil se mueva de verdad una primera vez, su vigencia pasa a ser
-- 'manual' o 'diagnostico_gps' y el recorte empieza a aplicarse como se diseñó.
--
-- Es un `create or replace` de la función: no toca ninguna tabla, ningún dato y
-- ninguna política. El resto del cuerpo es idéntico a 20260815110000.
-- ============================================================================
...
begin
  ...
  -- En un objetivo MÓVIL la ventana no puede cruzar una mudanza. Pero sólo
  -- cuenta como mudanza una vigencia que describe un cambio real: la
  -- incorporación inicial es la fecha en que empezamos a registrar el
  -- historial, no el día en que el objetivo llegó ahí.
  if v_objetivo.tipo_ubicacion = 'movil' then
    ...
    where u.objetivo_id = p_objetivo_id
      ...
      and u.origen <> 'incorporacion_inicial'
    limit 1;
  ...
  insert into public.objetivo_diagnosticos_gps (
    ...
    jsonb_build_object(
      ...
      'acotado_a_vigencia', v_desde_vig is not null,
