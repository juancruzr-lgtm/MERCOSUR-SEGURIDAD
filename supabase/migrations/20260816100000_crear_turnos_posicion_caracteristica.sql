-- crear_turnos_posicion_objetivo: elegir la caracteristica del turno
-- APLICADA EN PRODUCCION: 2026-08-13 (SQL editor)
--
-- Verificacion post: firma de 8 parametros terminando en p_tipo_evento text;
-- CHECK turnos_tipo_evento_check con (normal | cobertura | capacitacion).
-- Estado de los datos al aplicar: normal 4701 · cobertura 3 · capacitacion 3.
--
-- La creacion desde la grilla insertaba siempre tipo_evento = 'normal'. Las
-- capacitaciones solo se podian cargar de a una desde el alta manual de
-- Turnos, aunque una capacitacion tipica es un bloque de varios dias: la misma
-- operacion se podia hacer por una pantalla y por la otra no. Es el mismo
-- desfasaje que ya se corrigio con p_permitir_duplicado.
--
-- Se agrega p_tipo_evento. Por defecto 'normal': cualquier llamada existente
-- se comporta exactamente igual que antes.
--
-- Cambia tambien el criterio de duplicado. Antes buscaba un turno vigente
-- 'normal' en esa posicion, fecha y horario. Ahora busca uno de la MISMA
-- caracteristica que se esta creando:
--   · programar una capacitacion donde ya hay un turno normal es legitimo
--     —son cosas distintas, una se cobra al objetivo y la otra no—;
--   · cargar dos veces la misma capacitacion sigue bloqueado.
-- Para p_tipo_evento = 'normal' el comportamiento es identico al anterior.
--
-- Lo que NO cambia: una capacitacion se le paga al vigilador y no se le cobra
-- al objetivo. Esta funcion solo crea el turno; el descuento vive en las
-- planillas y resumenes (lib/caracteristica-turno.ts).
--
...
-- Rollback: supabase/rollback/20260816100000_crear_turnos_posicion_caracteristica_rollback.sql

DROP FUNCTION IF EXISTS public.crear_turnos_posicion_objetivo(uuid, uuid, uuid, time, time, jsonb);
DROP FUNCTION IF EXISTS public.crear_turnos_posicion_objetivo(uuid, uuid, uuid, time, time, jsonb, boolean);
...
CREATE OR REPLACE FUNCTION public.crear_turnos_posicion_objetivo(
  ...
  p_tipo_evento       text DEFAULT 'normal'
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.crear_turnos_posicion_objetivo(uuid, uuid, uuid, time, time, jsonb, boolean, text) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.crear_turnos_posicion_objetivo(uuid, uuid, uuid, time, time, jsonb, boolean, text) TO authenticated;
