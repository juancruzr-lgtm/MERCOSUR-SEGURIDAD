-- ============================================================================
-- LIQ3/permisos — Administración puede PREPARAR Liquidación (no económico/banco)
-- ============================================================================
-- puede_liquidar_actual() = Gerencia O Administración. Swap de 12 políticas + 6
-- RPC (cuerpo EXACTO actual, sólo el gate). Sergio/Dir.Op/Jefe/superv./vigil. NO.
-- Acceso a Liquidación NO implica banco/Finanzas. No usa rol admin legacy.
-- ROLLBACK: supabase/rollback/20260909100000_liq3_permisos_administracion_rollback.sql
-- ============================================================================

create or replace function public.puede_liquidar_actual() returns boolean
...
$fn$;
revoke all on function public.puede_liquidar_actual() from public, anon;
grant execute on function public.puede_liquidar_actual() to authenticated;

drop policy if exists liquidacion_ajuste_gerencia on public.liquidacion_ajuste;
create policy liquidacion_ajuste_liquidacion on public.liquidacion_ajuste for all to authenticated using (public.puede_liquidar_actual()) with check (public.puede_liquidar_actual());
drop policy if exists liquidacion_auditoria_gerencia on public.liquidacion_auditoria;
create policy liquidacion_auditoria_liquidacion on public.liquidacion_auditoria for all to authenticated using (public.puede_liquidar_actual()) with check (public.puede_liquidar_actual());
drop policy if exists liquidacion_concepto_catalogo_gerencia on public.liquidacion_concepto_catalogo;
create policy liquidacion_concepto_catalogo_liquidacion on public.liquidacion_concepto_catalogo for all to authenticated using (public.puede_liquidar_actual()) with check (public.puede_liquidar_actual());
drop policy if exists liquidacion_concepto_periodo_gerencia on public.liquidacion_concepto_periodo;
create policy liquidacion_concepto_periodo_liquidacion on public.liquidacion_concepto_periodo for all to authenticated using (public.puede_liquidar_actual()) with check (public.puede_liquidar_actual());
drop policy if exists liquidacion_concepto_permanente_gerencia on public.liquidacion_concepto_permanente;
create policy liquidacion_concepto_permanente_liquidacion on public.liquidacion_concepto_permanente for all to authenticated using (public.puede_liquidar_actual()) with check (public.puede_liquidar_actual());
drop policy if exists liquidacion_consolidada_gerencia on public.liquidacion_consolidada;
create policy liquidacion_consolidada_liquidacion on public.liquidacion_consolidada for all to authenticated using (public.puede_liquidar_actual()) with check (public.puede_liquidar_actual());
drop policy if exists liquidacion_dias_gerencia on public.liquidacion_dias;
create policy liquidacion_dias_liquidacion on public.liquidacion_dias for all to authenticated using (public.puede_liquidar_actual()) with check (public.puede_liquidar_actual());
drop policy if exists liquidacion_expediente_gerencia on public.liquidacion_expediente;
create policy liquidacion_expediente_liquidacion on public.liquidacion_expediente for all to authenticated using (public.puede_liquidar_actual()) with check (public.puede_liquidar_actual());
drop policy if exists liquidacion_importacion_gerencia on public.liquidacion_importacion;
create policy liquidacion_importacion_liquidacion on public.liquidacion_importacion for all to authenticated using (public.puede_liquidar_actual()) with check (public.puede_liquidar_actual());
drop policy if exists liquidacion_periodo_gerencia on public.liquidacion_periodo;
create policy liquidacion_periodo_liquidacion on public.liquidacion_periodo for all to authenticated using (public.puede_liquidar_actual()) with check (public.puede_liquidar_actual());
drop policy if exists liquidacion_periodo_empleado_gerencia on public.liquidacion_periodo_empleado;
create policy liquidacion_periodo_empleado_liquidacion on public.liquidacion_periodo_empleado for all to authenticated using (public.puede_liquidar_actual()) with check (public.puede_liquidar_actual());
drop policy if exists liquidacion_persona_gerencia on public.liquidacion_persona;
create policy liquidacion_persona_liquidacion on public.liquidacion_persona for all to authenticated using (public.puede_liquidar_actual()) with check (public.puede_liquidar_actual());
...
begin
  if auth.uid() is not null and not public.puede_liquidar_actual() then
    raise exception 'Sólo Gerencia/Administración puede comparar liquidaciones';
  end if;
...
begin
  if v_uid is not null and not public.puede_liquidar_actual() then
    raise exception 'Sólo Gerencia/Administración puede consolidar liquidaciones';
  end if;
  select id into v_actor from public.usuarios where auth_user_id = v_uid and estado='activo';
  select estado into v_estado from public.liquidacion_periodo where id = p_periodo_id;
  if not found then raise exception 'Período inexistente'; end if;
...
begin
  if v_uid is not null and not public.puede_liquidar_actual() then
    raise exception 'Sólo Gerencia/Administración puede importar conceptos de liquidación';
  end if;
  select id into v_actor from public.usuarios where auth_user_id = v_uid and estado='activo';
...
begin
  if v_uid is not null and not public.puede_liquidar_actual() then
    raise exception 'Sólo Gerencia/Administración puede exportar a Visual';
  end if;
  select id into v_actor from public.usuarios where auth_user_id = v_uid and estado='activo';
  select estado into v_estado from public.liquidacion_periodo where id = p_periodo_id;
  if not found then raise exception 'Período inexistente'; end if;
