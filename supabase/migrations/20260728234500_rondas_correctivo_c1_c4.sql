/*
================================================================================
CORRECTIVO RONDAS C1/C4 — configuración GPS e inicio idempotente
================================================================================

ALCANCE
  * Exige configuración completa cuando un punto requiere GPS.
  * Impide que un snapshot GPS incompleto produzca un punto cumplido.
  * Distingue recuperar la misma ronda de encontrar otra ronda en curso.

NO INCLUYE
  Precisión GPS en el veredicto, omisión, cancelación, cierre administrativo,
  interfaz, supervisor, alertas, reportes ni saneamiento de datos.

ROLLBACK
  supabase/rollback/20260728234500_rondas_correctivo_c1_c4_rollback.sql
================================================================================
*/

begin;

-- ── Precondición: nunca corregir datos de forma implícita ───────────────────

do $$
declare
  v_ronda_puntos_invalidos             bigint;
  v_ejecucion_puntos_snap_invalidos    bigint;
begin
  select count(*)
    into v_ronda_puntos_invalidos
    from public.ronda_puntos
   where gps_requerido
     and (
       latitud is null
       or longitud is null
       or radio_metros is null
     );

  select count(*)
    into v_ejecucion_puntos_snap_invalidos
  ...
  if v_ronda_puntos_invalidos > 0 or v_ejecucion_puntos_snap_invalidos > 0 then
    raise exception
      'MIGRACIÓN BLOQUEADA: configuración GPS inválida; public.ronda_puntos=% registro(s), public.ronda_ejecucion_puntos=% snapshot(s). Se requiere saneamiento previo explícitamente autorizado.',
      v_ronda_puntos_invalidos,
      v_ejecucion_puntos_snap_invalidos;
  end if;
end;
$$;

-- ── C1: invariantes persistentes ────────────────────────────────────────────

alter table public.ronda_puntos
  add constraint ronda_puntos_gps_config_completa
  check (
    not gps_requerido
    or (
      latitud is not null
      and longitud is not null
      and radio_metros is not null
    )
  );

alter table public.ronda_ejecucion_puntos
  add constraint ronda_ejecucion_puntos_snap_gps_config_completa
  check (
    not snap_gps_requerido
    or (
      snap_latitud is not null
      and snap_longitud is not null
      and snap_radio_metros is not null
    )
  );

-- ── C4: iniciar o recuperar con identidad de ronda ──────────────────────────
...
-- ── C1: defensa transaccional del registro de punto ─────────────────────────
