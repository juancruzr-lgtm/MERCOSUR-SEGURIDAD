-- ============================================================================
-- ALCANCE DE ESCRITURA DE TURNOS/PROGRAMACIÓN — ROLES 2b (con ROLES 3)
-- ============================================================================
-- Reemplaza el gate de scope por-rol de las RPC de escritura por el alcance
-- canónico (alcance_operativo_de): quien tiene alcance 'zonas_asignadas'
-- (p.ej. Sergio, admin de identidad / supervisor de puesto) sólo puede escribir
-- sobre objetivos de sus zonas; 'todas' (jefe/dir_op/administracion/gerencia)
-- sin restricción de zona; el gate de ENTRADA por rol y la regla admin-re-cierre
-- quedan para ROLES 4. Sin zonas asignadas ⇒ nada (COALESCE, vacío deniega).
--
-- Va junto con la reconciliación de VISTAS del frontend (ROLES 3) para no dejar
-- superficies "ve pero no puede": las 6 RPC de escritura (incluida cerrar_turno,
-- cuya vista Planillas se reconcilia en la misma entrega).
--
-- alcanza_objetivo_actual(objetivo): predicado por auth.uid() (otorgable a
-- authenticated) para usar en RLS de programación/auditoría.
--
-- ROLLBACK: supabase/rollback/20260908130000_alcance_escritura_turnos_rollback.sql
-- VERIFICACIÓN: matriz automatizada en tests/authz (simula contexto + ROLLBACK).
-- ============================================================================

-- Predicado canónico por sesión actual (para RLS/clientes)
create or replace function public.alcanza_objetivo_actual(p_objetivo_id uuid)
...
$$;
revoke all on function public.alcanza_objetivo_actual(uuid) from public;
revoke all on function public.alcanza_objetivo_actual(uuid) from anon;
grant execute on function public.alcanza_objetivo_actual(uuid) to authenticated;


-- ── publicar_turnos_programacion ──
                                                                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                                                                            IF NOT FOUND THEN
                                                                                                                                                                                                                                                                                                                RAISE EXCEPTION 'No autorizado: la publicacion de programacion es de administracion o supervision';
                                                                                                                                                                                                                                                                                                                  END IF;
                                                                                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                                                                                    v_alcance := public.alcance_operativo_de(v_actor.id);
  IF v_alcance = 'zonas_asignadas' THEN
                                                                                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                                                                                            SELECT id, estado, zona_id INTO v_objetivo FROM public.objetivos WHERE id = p_objetivo_id;
                                                                                                                                                                                                                                                                                                                              IF NOT FOUND THEN
                                                                                                                                                                                                                                                                                                                                  RAISE EXCEPTION 'Objetivo inexistente';
                                                                                                                                                                                                                                                                                                                                    END IF;
                                                                                                                                                                                                                                                                                                                                      IF v_alcance = 'zonas_asignadas' AND NOT (v_objetivo.zona_id = ANY (COALESCE(v_zonas, '{}'::uuid[]))) THEN
...
AS $function$
declare
  v_uid uuid; v_actor record; v_zonas uuid[];
  v_alcance   text; v_guardia record; v_tid uuid; v_turno record;
...
begin
  ...
  foreach v_tid in array p_turno_ids loop
    ...
    begin
      ...
      if v_alcance = 'zonas_asignadas' AND NOT (v_turno.objetivo_zona = ANY (COALESCE(v_zonas, '{}'::uuid[]))) then
...
declare
  v_uid uuid; v_actor record; v_zonas uuid[];
  v_alcance   text; v_tid uuid; v_turno record;
...
-- ── crear_turnos_posicion_objetivo ──
...
-- ── RLS: programación y auditoría de puestos por alcance canónico ────────────
alter policy programacion_publicaciones_select on public.programacion_publicaciones
  using ( public.alcanza_objetivo_actual(programacion_publicaciones.objetivo_id) );
alter policy puestos_auditoria_select on public.puestos_auditoria
  using ( public.alcanza_objetivo_actual(puestos_auditoria.objetivo_id) );
