-- Bloque E — Impedir generación retroactiva de turnos
--
-- Regla: la programación mensual no crea turnos para fechas anteriores al
-- día de ejecución (hora Argentina, UTC-3 fija como en el resto del
-- proyecto). El día actual solo es generable si el turno todavía no comenzó.
-- Los días pasados se resuelven por regularización administrativa
-- (Reportes / Bandeja de Diferencias / Crear registro), nunca desde acá.
--
-- La validación vive también en servidor (no se confía en la interfaz):
-- una fila pasada enviada a mano vuelve como omitida con motivo
-- 'fecha_pasada', sin abortar el resto del lote.
--
-- CREATE OR REPLACE de crear_turnos_programacion_parcial (misma firma,
-- mismos permisos). Único cambio: el chequeo de fecha pasada por fila.
